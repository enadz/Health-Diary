...
public interface PatientActivityDataRepository extends CrudRepository<PatientActivityData, Integer> {
    @Query(value = "SELECT * FROM step_data WHERE patient_id = ?1", nativeQuery = true)
    public Iterable<PatientActivityData> findByPatientId(@Param("patientId") int patientId);
}
