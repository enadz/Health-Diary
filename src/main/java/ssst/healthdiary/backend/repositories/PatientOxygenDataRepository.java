...
public interface PatientOxygenDataRepository extends CrudRepository<PatientOxygenData, Integer> {
    ...
    @Query(value = "SELECT * FROM oxygen_saturation_data WHERE patient_id = ?1", nativeQuery = true)
    public Iterable<PatientOxygenData> findByPatientId(@Param("patientId") int patientId);
}
